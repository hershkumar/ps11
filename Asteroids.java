...
class Asteroids extends Game {
	...
	public void paint(Graphics brush) {
		...
		for(int i= 0; i<listAstSize-1; i++) {
			for (int g = 0; g<listBullSize-1;g++) {
				if (listBull.get(g).collides(listAst.get(i))) {
	...
	public void breakAst(Asteroid a) {
		...
		if (size > 900) {
			...
			Asteroid newAst2 = new Asteroid(smallAstPoints,a.position,a.rotation);
			listAst.add(newAst);
			listAst.add(newAst2);
		...
		if (size == 400) {
			Asteroid newAst = new Asteroid(smallerAstPoints,a.position,a.rotation);
			Asteroid newAst2 = new Asteroid(smallerAstPoints,a.position,a.rotation);
			listAst.add(newAst);
			listAst.add(newAst2);
			int index = listAst.indexOf(a);
			listAst.remove(index);
